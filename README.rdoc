...
Please run `rspec` to run the program and spec suite or the file `shopicruit.rb` to run the program without the spec suite.

Assumptions made:
* All variants are available.
* Only one count of each variant is available.
* API response structure remains as retrieved on 22/2/2016.